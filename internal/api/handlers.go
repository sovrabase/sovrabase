...
func (s *Server) handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	// Fallback: if the storage driver didn't provide a public URL, construct one
	// pointing to the download endpoint.
	if info.URL == "" {
		scheme := "http"
		if r.TLS != nil {
			scheme = "https"
		}
		info.URL = fmt.Sprintf("%s://%s/api/v1/storage/%s/%s", scheme, r.Host, bucket, path)
	}
